...
### 修改为R4A百兆版Breed直刷版
## 1.修改 mt7628an_xiaomi_mi-router-4a-100m.dts
export shanchu1=$(grep  -a -n -e '&spi0 {' target/linux/ramips/dts/mt7628an_xiaomi_mi-router-4a-100m.dts|cut -d ":" -f 1)
export shanchu2=$(grep  -a -n -e '&pcie {' target/linux/ramips/dts/mt7628an_xiaomi_mi-router-4a-100m.dts|cut -d ":" -f 1)
export shanchu2=$(expr $shanchu2 - 1)
export shanchu2=$(echo $shanchu2"d")
sed -i $shanchu1,$shanchu2 target/linux/ramips/dts/mt7628an_xiaomi_mi-router-4a-100m.dts
grep  -Pzo '&spi0[\s\S]*};[\s]*};[\s]*};[\s]*};' target/linux/ramips/dts/mt7621_youhua_wr1200js.dts > youhua.txt
echo "" >> youhua.txt
echo "" >> youhua.txt
export shanchu1=$(expr $shanchu1 - 1)
export shanchu1=$(echo $shanchu1"r")
sed -i "$shanchu1 youhua.txt" target/linux/ramips/dts/mt7628an_xiaomi_mi-router-4a-100m.dts
rm -rf youhua.txt
## 2.修改mt76x8.mk
